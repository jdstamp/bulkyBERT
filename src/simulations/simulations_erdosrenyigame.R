...
generate_network <- function(n_genes, prob_edges) {
  ...
  activation_weights <-
    sample(c(1, -1), ecount(graph), rep = TRUE, p = c(.8, .2))
...
simulate_gene_expression <- function(graph, n_time_points) {
  ...
  reaction_rate_constants <-
    c(0.002, 0.005, 0.005, 0.005, 0.01, 0.02)
...
num_networks <- 2
prob_edges <- c(0.1, 0.2, 0.3)

stacked_expression <- NULL
stacked_labels <- NULL

for (p in 1:length(prob_edges)) {
  p_network <- prob_edges[p]
  network <- generate_network(n_genes, p_network)
  for (i in 1:num_replications) {
    exp_network <- simulate_gene_expression(network, n_time_points)
    stacked_expression <-
      abind(stacked_expression, exp_network, along = 3)
    network_labels <- rep(p, n_genes)
    stacked_labels <-
      abind(stacked_labels, network_labels, along = 2)
  }
}
